...
component name="package" extends="foundry.core.emitter" {
	public any function init(name, endpoint, manager)  {
		//var spawn    = require('child_process').spawn; (same as cfthread?)
		//var https    = require('https');
		//var http     = require('http'); (same as cfhttp?);
		//var fstream  = require('fstream'); maybe not needed?
		//var template = require('../util/template'); not using hogan templates
		//var readJSON = require('../util/read-json');
		//var url      = require('url'); (might not need this?)
		
		//NEEDED:
		var _        = require('UnderscoreCF');
		//var mkdirp   = require('mkdirp'); //not done yet
		var emitter  = require('emitter').init();
		//var rimraf   = require('rimraf'); //not done yet
		var semver   = require('semver');
		var async    = require('async');
		var regexp    = require('regexp');
		
		var path     = require('path');
		//var tmp      = require('tmp'); //not done yet
		var fs       = require('fs');
		...
		if(server.os.name CONTAINS "windows") {
			...
			cache = path.resolve((len(appdata) GT 0 ? appdata : temp), "bower-cache");
		} else {
			home = env.get('HOME');
			cache = path.resolve((len(home) GT 0? home : temp), ".bower");
		}
	  ...
	  if (structKeyExists(arguments,'endpoint')) {
	    ...
	    } else if (RegExp.setPattern("^(.*\.git)##(.*)$") && RegExp.test(endpoint)) {
	      this.tag    = RegExp.$2;
	      this.gitUrl = RegExp.$1.replace("^git\+", '');
	      
	    } else if (RegExp.setPattern("^(?:(git):|git\+(https?):)\/\/([^##]+)##?(.*)$") && RegExp.test(endpoint)) {
		...
		} else if (RegExp.setPattern("^[\.\/~]\.?[^.]*\.(js|css)") AND RegExp.test(endpoint) AND fileExists(endpoint)) {
	      this.path      = path.resolve(endpoint);
	      this.assetType = path.extname(endpoint);
	      this.name      = replace(name,this.assetType, '');

	    } else if (RegExp.setPattern("^[\.\/~]") AND RegExp.test(endpoint) AND fileExists(endpoint)) {
	      
	      this.path = path.resolve(endpoint);

	    } else if (RegExp.setPattern("^https?:\/\/") AND RegExp.test(endpoint)) {
